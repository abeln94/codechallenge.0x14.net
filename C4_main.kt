...
private fun run(data: String) {
    // prepare output file
    File("C4_${data}_out.txt").printWriter().use { out ->
        var index = 1
        // prepare input file
        File("C4_${data}_in.txt").readLines().drop(1).windowed(2, 2) { (f, s) ->
            // convert each
 ...
 * Convert each step
 */
private fun convert(root: String, scale: String): String {
...
//    return foldUnique(listOf(root), notes.drop(1)) ?: throw Exception("no solution") // replace the previous line with this one to 'fix' the problem
}
